...
public class SwipeBackLayout extends ViewGroup {
    ...
    private SwipeBackToRightListener swipeBackToRightListener;
    ...
    public void setOnSwipeBackToRightListener(SwipeBackToRightListener listener) {
        swipeBackToRightListener = listener;
    }
    ...
    protected void onLayout(boolean changed, int l, int t, int r, int b) {
        ...
        mAutoBackOriginPos.x = getLeft();
        mAutoBackOriginPos.y = getTop();
    ...
    public boolean onInterceptTouchEvent(MotionEvent ev) {
        try {
            if (getRangeView() != null && Utilities.inRangeOfView(getRangeView(), ev)) {
            ...
            int result = 0;
                if(isLeftOrRight){
                    if(!canChildScrollRight() &&left > 0){
                        ...
                        if(swipeBackToRightListener != null){
                            swipeBackToRightListener.onSwipeLeft();
                        }
                    }else if(!canChildScrollLeft() && left < 0){
                        dragEdge = DragEdge.RIGHT;
                        if(swipeBackToRightListener != null){
                            swipeBackToRightListener.onSwipeRight();
                        }
                    }
                }
        ...
        public void onViewDragStateChanged(int state) {
            ...
            if ((draggingState == ViewDragHelper.STATE_DRAGGING || draggingState == ViewDragHelper.STATE_SETTLING) &&
                ...
                if (draggingOffset == getDragRange()) {
                    Log.i("luffy", "onViewDragStateChanged: " + dragEdge);
                    if(isLeftOrRight && swipeBackToRightListener!=null && dragEdge == DragEdge.RIGHT){
                        swipeBackToRightListener.onSwipeRightFinish();
                    }else {
                        finish();
                    }
                }
    ...
    public void moveToOriginal(int x){
        try {
            setX(x);
           setScrollX(x);
            //setTranslationX(0);
    ...
    public interface SwipeBackToRightListener{
        void onSwipeRightFinish();
        void onSwipeRight();
        void onSwipeLeft();
    }
